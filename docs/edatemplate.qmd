---
title: '`r paste(params$yr, "exploratory analysis of Old Tampa Bay temperature data")`'
author: 
...
mapview(otbsub, zcol = NULL, layer.name = 'Sub-segments', col.regions = 'lightblue', homebutton = F, legend = T, map.types = 'Esri.WorldGrayCanvas') + 
  mapview(EPC, homebutton = F, col.regions = 'red', legend = T, alpha = 0) + 
  mapview(metadat |> filter(yr %in% params$yr), layer.name = paste(params$yr, ' logger data'), homebutton = F)
```
...
toplo <- bsdat %>% 
  ...
  mutate(
    ...
    .by  = subseg, 
    julmo = ifelse(mo %in% unique(datsub$mo), 'Deployment\nmonths', 'All other')
  ) 
