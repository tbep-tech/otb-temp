...
This document provides exploratory data analyses for the temperature logger deployments in Old Tampa Bay (OTB).  The graphs herein are meant to provide some quantitative assessments to address the following questions: 

1. Are there differences in temperature at sites where seagrasses were lost compared to where it was gained?  
1. Are these differences related to location in OTB, such that thermal stress may be higher along a spatial gradient (either along the longitudinal axis or in relation to hydrology as affected by land bridges)?  

The questions follow the general hypothesis that seagrass loss in OTB could partially be explained by thermal stress, where temperature increases in recent years could stress seagrasses beyond their thermal maxima.  I assume there's literature to back this up...
...
1. Temperature may differ given time of year or preceding weather, such that direct comparisons of data between deployment dates (and years) is not possible
     * Only compare within deployment dates
     * Or normalize the data by subtracting the mean temperature for each deployment date
...
1. Any statistical test must account for temporal correlation in the continuous monitoring data, such that summaries or subsets of the data 
...
ggplot(toplo, aes(x = subseg, y = tempc, group = subseg)) + 
  geom_point(aes(fill = julmo), position = position_dodge2(width = 0.4), alpha = 0.8, pch = 21, color  = "#FFFFFF00", size = 3) + 
...
All data for the current project can be viewed in the same plot that shows the individual time series by deployment date, statum, and OTB sub-segement. 
...
ggplot(toplo, aes(x = hr, y = stratum, fill = tempc)) + 
  ...
  scale_fill_gradient2(low = 'green', mid = 'yellow', high = 'red', na.value = 'white', midpoint = mean(toplo$tempc)) +
